...
class NewsAnalyzer:
    ...
    def _should_open_browser(self) -> bool:
        """判断是否应该打开浏览器"""
        # 如果设置了禁用环境变量，则不打开
        if os.environ.get("TRENDRADAR_NO_BROWSER") == "true":
            return False
    ...
    def _run_ai_analysis(
        ...
        if schedule.once_analyze and schedule.period_key:
            ...
            executed, cached_json = scheduler.already_executed(schedule.period_key, "analyze", date_str)
            if executed and cached_json:
                print(f"[AI] 调度器: 时间段 {schedule.period_name or schedule.period_key} 今天已分析过，尝试复用结果")
                try:
                    import json
                    from trendradar.ai.analyzer import AIAnalysisResult
                    cached_data = json.loads(cached_json)
                    result = AIAnalysisResult.from_dict(cached_data)
                    print(f"[AI] 已成功从数据库恢复缓存的分析结果")
                    return result
                except Exception as e:
                    print(f"[AI] 恢复缓存结果失败: {e}，将尝试重新分析")
            elif executed:
                print(f"[AI] 调度器: 时间段 {schedule.period_name or schedule.period_key} 今天已分析过但无缓存，将重新执行以建立缓存")
            else:
            ...
            if result.success:
                ...
                if schedule.once_analyze and schedule.period_key:
                    try:
                        ...
                        scheduler = self.ctx.create_scheduler()
                        date_str = self.ctx.format_date()
                        scheduler.record_execution(schedule.period_key, "analyze", date_str, result_json)
                    except Exception as e:
                        print(f"[AI] 记录 AI 分析结果失败: {e}")
            else:
