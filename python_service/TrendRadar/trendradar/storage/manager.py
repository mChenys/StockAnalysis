...
class StorageManager:
    ...
    def has_period_executed(self, date_str: str, period_key: str, action: str) -> tuple[bool, Optional[str]]:
        """检查指定时间段的某个 action 是否已执行"""
        return self.get_backend().has_period_executed(date_str, period_key, action)

    def record_period_execution(self, date_str: str, period_key: str, action: str, result_json: Optional[str] = None) -> bool:
        """记录时间段的 action 执行"""
        return self.get_backend().record_period_execution(date_str, period_key, action, result_json)
