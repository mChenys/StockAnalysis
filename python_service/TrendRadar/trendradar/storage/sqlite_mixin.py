...
class SQLiteStorageMixin:
    ...
    def _has_period_executed_impl(self, date_str: str, period_key: str, action: str) -> tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            # 额外检查一次是否已经有 result_json 列 (平滑升级)
            try:
                cursor.execute("""
                    SELECT 1 FROM period_executions LIMIT 1
                """)
                # 如果没报错说明表存在，尝试读取 result_json
                cursor.execute("""
                    SELECT 1 FROM pragma_table_info('period_executions') WHERE name = 'result_json'
                """)
                has_column = cursor.fetchone() is not None
                
                if not has_column:
                    # 如果列不存在，动态添加（平滑升级旧数据库）
                    cursor.execute("ALTER TABLE period_executions ADD COLUMN result_json TEXT")
            except:
                pass

            cursor.execute("""
                SELECT 1, result_json FROM period_executions
    ...
    def _record_period_execution_impl(self, date_str: str, period_key: str, action: str, result_json: Optional[str] = None) -> bool:
        """
        记录时间段的 action 执行
        ...
        try:
            ...
            # 平滑升级：如果表已存在但没列，增加列
            try:
                cursor.execute("SELECT result_json FROM period_executions LIMIT 1")
            ...
            cursor.execute("""
                INSERT INTO period_executions (execution_date, period_key, action, result_json, executed_at)
                VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(execution_date, period_key, action) DO UPDATE SET
                    result_json = excluded.result_json,
                    executed_at = excluded.executed_at
            """, (date_str, period_key, action, result_json, now_str))
