    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; -webkit-font-smoothing: antialiased; }
            ...
            .word-count.hot { background: #fee2e2; color: #ef4444; }
            .word-count.warm { background: #ffedd5; color: #f97316; }
            ...
            .feed-name { font-weight: 700; color: #065f46; }
            ...
            .standalone-section-title::before {
                content: "";
                display: inline-block;
                width: 4px;
                height: 20px;
            ...
            .ai-block-content { font-size: 14px; color: #334155; line-height: 1.7; }
            ...
            .footer-content { font-size: 13px; color: var(--text-muted); font-weight: 500; }
            .footer-link { color: var(--primary); text-decoration: none; font-weight: 700; }

            @media (max-width: 480px) {
                .header { padding: 40px 20px; }
                .header-info { grid-template-columns: repeat(2, 1fr); }
                .save-buttons { position: static; margin-bottom: 20px; flex-direction: column; width: 100%; }
                .save-btn { width: 100%; }
                .content { padding: 20px; }
                .news-item { padding: 16px; flex-direction: column; gap: 8px; }
            }
            .layout-grid {
                display: grid;
        ...
        <div class="container">
            <div class="header">
                <div class="save-buttons">
                    <button class="save-btn" onclick="addToFavorites()">加入收藏</button>
                </div>
        ...
        for platform in platforms:
            ...
            standalone_html += f"""
                    <div class="word-group">
                        <div class="word-header">
                            <div class="word-info">
                                <div class="word-name">{html_escape(platform_name)}</div>
                                <div class="word-count">{len(items)} 条</div>
                            </div>
                        </div>"""
        ...
        for feed in rss_feeds:
            ...
            standalone_html += f"""
                    <div class="word-group">
                        <div class="word-header">
                            <div class="word-info">
                                <div class="word-name">{html_escape(feed_name)}</div>
                                <div class="word-count">{len(items)} 条</div>
                            </div>
                        </div>"""
    ...
    for region in region_order:
        ...
        if region == "ai_analysis":
            ...
            if new_html:
                ...
                main_col_html += new_html
                has_previous_content = True
            if rss_new:
                if has_previous_content:
                    rss_new = add_section_divider(rss_new)
                main_col_html += rss_new
                has_previous_content = True
        else:
            if content:
                if has_previous_content:
                    content = add_section_divider(content)
                main_col_html += content
                has_previous_content = True

    if sidebar_html:
        html += '<div class="layout-grid">'
        html += f'<div class="main-column">{main_col_html}</div>'
        html += f'<div class="sidebar-column">{sidebar_html}</div>'
        html += '</div>'
    else:
        html += main_col_html
    ...
    html += """
        ...
        <script>
            ...
            function addToFavorites() {
                ...
                const timeElement = document.querySelector('.info-value:last-child');
                const titleText = (titleElement ? titleElement.innerText : '热点报告') + ' - ' + (timeElement ? timeElement.innerText : '');
                ...
                const saveBtns = contentClone.querySelector('.save-buttons');
                if (saveBtns) saveBtns.remove();
                
                // 发送消息给父窗口
                window.parent.postMessage({
                    action: 'SAVE_TO_FAVORITES',
                    type: 'trendradar',
                    title: '热点雷达: ' + titleText,
                    content: contentClone.innerHTML
                }, '*');

                setTimeout(() => {
                    btn.textContent = '已加入收藏';
