<script lang="ts">
	...
	onMount(() => {
		...
		const cleanupWs = startMarketWebSockets();
		...
		<Dashboard>
			...
			{#if isPanelVisible('politics')}
				<div class="panel-slot">
					<NewsPanel category="politics" panelId="politics" title="全球地缘政治 (POLITICS)" />
				</div>
			{/if}

			{#if isPanelVisible('tech')}
				<div class="panel-slot">
					<NewsPanel category="tech" panelId="tech" title="前沿科技与AI (TECH)" />
				</div>
			{/if}

			{#if isPanelVisible('finance')}
				<div class="panel-slot">
					<NewsPanel category="finance" panelId="finance" title="全球金融市场 (FINANCE)" />
				</div>
			{/if}

			{#if isPanelVisible('gov')}
				<div class="panel-slot">
					<NewsPanel category="gov" panelId="gov" title="政府政策与监管 (GOVERNMENT)" />
				</div>
			{/if}

			{#if isPanelVisible('ai')}
				<div class="panel-slot">
					<NewsPanel category="ai" panelId="ai" title="AI 军备竞赛边界 (AI)" />
				</div>
			{/if}
			...
			{#if isPanelVisible('printer')}
				<div class="panel-slot">
					<PrinterPanel
						data={$fedIndicators.data
							? {
									value: $fedIndicators.data.balanceSheet.value || 0,
									change: $fedIndicators.data.balanceSheet.change || 0,
									changePercent:
										(($fedIndicators.data.balanceSheet.change || 0) /
											(($fedIndicators.data.balanceSheet.previousValue || 1) )) *
										100,
									percentOfMax:
										(($fedIndicators.data.balanceSheet.value || 0) / 9) * 100 // Scale against 9T peak
								}
							: null}
