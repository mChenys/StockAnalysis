...
export async function fetchCategoryNews(category: NewsCategory): Promise<NewsItem[]> {
	...
	try {
		...
		// Build the raw GDELT URL with timespan=1d to get fresh articles faster
		const gdeltUrl = `https://api.gdeltproject.org/api/v2/doc/doc?query=${fullQuery}&timespan=1d&mode=artlist&maxrecords=12&format=json&sort=date`;
...
export async function fetchAllNews(): Promise<Record<NewsCategory, NewsItem[]>> {
	...
	// Fetch categories sequentially to avoid hitting GDELT rate limits
	// Parallel requests often trigger 429 or timeouts from their API
	for (const category of NEWS_CATEGORIES) {
		try {
			...
			logger.error('News API', `Failed to fetch ${category} in sweep:`, error);
		}
	}
