...
export interface FedIndicators {
	...
	balanceSheet: EconomicIndicator;
}
 ...
 * Series: WALCL (weekly)
 */
async function fetchBalanceSheet(): Promise<EconomicIndicator> {
	const seriesId = 'WALCL';
	const name = 'Fed Balance Sheet';
	const unit = 'T USD';
	...
	// Convert from Millions to Trillions for the UI
	const currentT = current !== null ? current / 1000000 : null;
	const previousT = previous !== null ? previous / 1000000 : null;

	return {
		seriesId,
		name,
		value: currentT,
		previousValue: previousT,
		change: currentT !== null && previousT !== null ? currentT - previousT : null,
		unit,
		date: observations[0]?.date || null
	};
}
...
export async function fetchFedIndicators(): Promise<FedIndicators> {
	...
	const [fedFundsRate, cpi, treasury10Y, balanceSheet] = await Promise.all([
		fetchFedFundsRate(),
		fetchCPI(),
		fetchTreasury10Y(),
		fetchBalanceSheet()
	...
	return { fedFundsRate, cpi, treasury10Y, balanceSheet };
}
...
async function fetchFedRssFeed(
	...
	try {
		...
		if (contentType?.includes('application/json')) {
			...
			xml = typeof data === 'string' ? data : (data.error ? '' : JSON.stringify(data));
		} else {
