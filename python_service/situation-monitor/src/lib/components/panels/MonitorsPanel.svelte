...
<Panel id="monitors" title="我的私塾雷达监控" {count} {loading} {error}>
	<div class="monitors-content">
		{#if monitors.length === 0 && !loading && !error}
			<div class="empty-state">
				<p>暂无您自选的定制化监控选项</p>
				{#if onCreateMonitor}
					<button class="create-btn" onclick={onCreateMonitor}> + 新建监控选项 </button>
				{/if}
			</div>
		{:else}
			<div class="monitors-header">
				<span class="active-count">{activeMonitors.length} 个运行中</span>
				{#if onCreateMonitor}
