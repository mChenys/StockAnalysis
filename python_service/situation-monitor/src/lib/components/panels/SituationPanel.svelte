<script lang="ts">
	...
	function calculateThreatLevel(
		...
		if (newsItems.length === 0) {
			return { level: 'monitoring', text: '日常监控 (MONITORING)' };
		}
		...
		if (hasCritical || recentNews.length >= 3) {
			return { level: 'critical', text: '爆发级别 (CRITICAL)' };
		}
		if (recentNews.length >= 1) {
			return { level: 'elevated', text: '加剧状态 (ELEVATED)' };
		}
		return { level: 'monitoring', text: '日常监控 (MONITORING)' };
	}
</script>
...
>
	<div class="situation-content">
		...
		{#if news.length === 0 && !loading && !error}
			<div class="empty-state">暂无最新动态</div>
		{:else}
