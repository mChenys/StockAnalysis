...
<Modal {open} title="设置界面" {onClose}>
	<div class="settings-sections">
		<section class="settings-section">
			<h3 class="section-title">启用面板配置</h3>
			<p class="section-desc">开启或关闭相应的面板以自定义您的雷达控制台</p>
		...
		<section class="settings-section">
			<h3 class="section-title">大盘预设配置</h3>
			{#if onReconfigure}
				<button class="reconfigure-btn" onclick={onReconfigure}> 重新选择大盘预设 (返回初始向导) </button>
				<p class="btn-hint">重新选择一个适合您需求的预设布局和关注焦点</p>
			{/if}
			<button class="reset-btn" onclick={handleResetPanels}> 恢复默认设置 </button>
		</section>
	</div>
</Modal>
