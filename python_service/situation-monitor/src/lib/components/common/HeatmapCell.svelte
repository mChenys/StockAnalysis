<script lang="ts">
	...
	const isDataAvailable = $derived(!isNaN(sector.changePercent) && sector.changePercent !== null);
	const colorClass = $derived(isDataAvailable ? getColorClass(sector.changePercent) : 'unavailable');
	...
	const changeText = $derived(isDataAvailable ? formatPercentChange(sector.changePercent) : '获取失败');
</script>

<a
	href="https://finance.yahoo.com/quote/{encodeURIComponent(sector.symbol)}"
	target="_blank"
	rel="noopener noreferrer"
	class="heatmap-cell {colorClass}"
>
...
<style>
	...
	.heatmap-cell.unavailable:hover {
